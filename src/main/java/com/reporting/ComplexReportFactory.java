...
public class ComplexReportFactory {
	...
	/*
	 * At any given instance there will be only one test running in a thread. We are
	 * already maintaining a thread to extentest map. This method should be used
	 * after some part of the code has already called getTest with proper testcase
	 ...
	 * from there. Utility function or similar code sections are not bound to a test
	 * hence we cannot get a reporter via test name, unless we pass test name in all
	 * method calls. Which will be an overhead and a rigid design. However, there is
	 * one common association which is the thread ID. When testng executes a test it
	 * puts it launches a new thread, assuming test level threading, all tests will
	 * have a unique thread id hence call to this function will return the right
	 * extent test to use
	 */
	public synchronized static ExtentTest getTest() {
		...
		// system log, this shouldnt happen but in this crazy times if it did happen log
		// it.
		return null;
	}
